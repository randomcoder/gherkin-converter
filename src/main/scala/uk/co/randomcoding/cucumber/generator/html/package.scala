...
package object html {
  ...
  def linkTextFromName(name: String) = {
    val capsFollowedByLower = """([A-Z0-9][a-z0-9])"""
    val lowerFollowedByCaps = """([a-z])([A-Z])"""
    name.takeWhile(_ != '.').replaceAll(capsFollowedByLower, """ $1""").replaceAll(lowerFollowedByCaps, """$1 $2""").trim
  }
}
